...
Channels and testers can now be configured entirely from Discord — no `.env` editing or
redeploy required:
...
`PRIVATE_QUEUE_CHANNEL_ID`, `WAITLIST_ROLE_ID`, and `STAFF_ROLE_IDS` are still set via `.env`
(see step 2 below), since those rarely change and require role/permission setup anyway.

**If `/startqueue` shows the confirmation but no embed appears in the channel**, or `/results`
replies with an error — that means the relevant channel hasn't been set yet. Run
`/setqueuechannel` / `/setresultchannel` first.
