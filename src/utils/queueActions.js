...
async function joinQueue(interaction) {
  ...
  const waitlistRoleId = runtimeConfig.getWaitlistRoleId();
  const member = interaction.member;

  await roleManager.giveWaitlistRole(member, waitlistRoleId);
  const gotChannelAccess = await channelManager.grantPrivateAccess(interaction.guild, member, privateQueueChannelId);
  ...
  if (!privateQueueChannelId) {
    await interaction.reply({
      content: '✅ You have joined the waitlist. ⚠️ No private queue channel is configured yet, so I couldn\'t grant you access to one - staff should run `/setprivatequeuechannel`.',
      ephemeral: true,
    });
  } else if (!gotChannelAccess) {
    await interaction.reply({
      content: '✅ You have joined the waitlist. ⚠️ I couldn\'t grant you access to the private channel - I probably don\'t have the "Manage Roles" permission there. Ping staff to check.',
      ephemeral: true,
    });
  } else {
  ...
  try {
    if (privateQueueChannelId && gotChannelAccess) {
